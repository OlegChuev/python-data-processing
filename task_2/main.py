...
#
# Returns array of times, heights, heights_accuracy, latitude and longitude.
def extract_data(data):
    ...
    latitude = [line[LATITUDE_POSITION] for line in split_lines]
    longitude = [line[LONGITUDE_POSITION] for line in split_lines]

    return times, heights, heights_accuracy, latitude, longitude


# Convert data
#
# Returns converted time, height, height_accuracy, latitude and longitude.
def convert_data(times, heights, heights_accuracy, latitude, longitude):
    ...
    latitude_conv = np.array([float(l) for l in latitude])
    longitude_conv = np.array([float(l) for l in longitude])

    return times_conv, heights_conv, heights_accuracy_conv, latitude_conv, longitude_conv
...
# Generates 3D trajectory chart
def generate_3d_chart(latitude_s, longitude_s, latitude, longitude, heights):
    # Trajectory diagram
    fig = plt.figure(figsize=(FIGSIZE, FIGSIZE))
    ax = fig.add_subplot(111, projection="3d")

    ax.scatter(latitude, longitude, heights)
    ...
    save_chart_to_file(plt, './output_4')
...
def _main():
    # Data preparation
    data = upload_file()
    times, heights, h_accuracy, latitude, longitude = extract_data(data)

    # Data conversion
    times_conv, heights_conv, heights_accuracy_conv, latitude_conv, longitude_conv = convert_data(times, heights, h_accuracy, latitude, longitude)
    ...
    generate_3d_chart(latitude, longitude, latitude_conv, longitude_conv, heights_conv)
